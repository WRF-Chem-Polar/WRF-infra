...
def get_wrf_proj(WRF_FILENAME):
  ...
  class wrf_projection():
    def __init__(self, map_proj, imax, jmax, kmax, dx, dy, moad_cen_lat, truelat1,
        truelat2, stdlon, ref_lat, ref_lon, ref_i, ref_j, pole_lat, pole_lon,
        hemi, mminlu, eta_u, p_top):
  ...
  wrf_proj = wrf_projection(map_proj, imax, jmax, kmax, dx, dy, moad_cen_lat, 
    truelat1, truelat2, stdlon, ref_lat, ref_lon, ref_i, ref_j, pole_lat,
    pole_lon, hemi, mminlu, eta_u, p_top)
  # Return wrf_proj
  return wrf_proj
...
def wrf_llij(lat, lon, wrf_proj):
  ...
  # Subroutines set_lc, lc_cone and llij_lc from WPS: geogrid/src/module_map_utils.f90
    ...
    raise ValueError('wrf_proj.map_proj={} invalid'.format(wrf_proj.map_proj))
...
def wrf_ijll(wrfi, wrfj, wrf_proj):
  ...
  if(wrf_proj.map_proj == 1):
    ...
    wrflon[r2!=0.0] = wrf_proj.stdlon + deg_per_rad \
                      * np.arctan2(wrf_proj.hemi * xx[r2!=0.0],yy[r2!=0.0]) / cone
    ...
    wrflon[wrflon > 180.] = wrflon[wrflon > 180.] - 360.
    wrflon[wrflon < -180.] = wrflon[wrflon < -180.] + 360.
    ...
    if(wrf_proj.ref_lat != 0.):
       rsw = (np.log(np.tan(0.5*((wrf_proj.ref_lat+90.)*rad_per_deg))))/dlon
