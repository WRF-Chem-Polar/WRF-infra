...
wrf_xland[wrf_xice > 0.] = 2.
# Calculate WRF grid edges
wrf_lat_edge, wrf_lon_edge = calc_wrf_grid_edges(wrf_proj)
minlat = np.min(wrf_lat_edge)-1.
maxlat = np.max(wrf_lat_edge)+1.
minlon = np.min(wrf_lon_edge)-1.
maxlon = np.max(wrf_lon_edge)+1.
if(minlat < -85.):
  minlat = -90.
if(maxlat > 85.):
  maxlat = 90.
...
print('cdo sellonlatbox,{},{},{},{} {} ERODFILE_subset.nc'.format(minlon,maxlon,minlat,maxlat,ERODFILE))
os.system('cdo sellonlatbox,{},{},{},{} {} ERODFILE_subset.nc'.format(minlon,maxlon,minlat,maxlat,ERODFILE))
# Regrid
print('Regrid file to WRF grid')
print('cdo remapcon,{} ERODFILE_subset.nc {}'.format(cdo_wrfgrid_file,erodfile_regrid))
error_code = os.system('cdo remapcon,{} ERODFILE_subset.nc {}'.format(cdo_wrfgrid_file,erodfile_regrid))
...
# Delete temp files
os.system('rm -f "{}" "{}" "ERODFILE_subset.nc"'.format(erodfile_regrid, cdo_wrfgrid_file))
