...
def create_cdo_wrfgridfile(WRFOUT_DOMAIN_FILE):
  ...
  with open(cdo_wrfgrid_file, 'a') as file:
      file.write('# WRF grid for CDO remapcon, from file '+WRFOUT_DOMAIN_FILE+'\n')
      file.write('gridtype  = curvilinear\n')
      file.write('gridsize  = '+str((wrf_proj.imax)*(wrf_proj.jmax))+'\n')
      ...
      for jj in range(wrf_proj.jmax):
        for ii in range(wrf_proj.imax):
          wrflat_ll,wrflon_ll = wrf_ijll([ii+1-0.5], [jj+1-0.5], wrf_proj)
          wrflat_lr,wrflon_lr = wrf_ijll([ii+1+0.5], [jj+1-0.5], wrf_proj)
          wrflat_ul,wrflon_ul = wrf_ijll([ii+1-0.5], [jj+1+0.5], wrf_proj)
          wrflat_ur,wrflon_ur = wrf_ijll([ii+1+0.5], [jj+1+0.5], wrf_proj)
          ...
          file.write(wrflon_lr_str+' '+wrflon_ur_str+' '+wrflon_ul_str+' '+wrflon_ll_str)
          valindex = valindex+1
          if(valindex == 1):
            file.write('\n             ')
            valindex = 0
      ...
      for jj in range(wrf_proj.jmax):
        for ii in range(wrf_proj.imax):
          ...
          file.write(wrflat_lr_str+' '+wrflat_ur_str+' '+wrflat_ul_str+' '+wrflat_ll_str)
          valindex = valindex+1
          if(valindex == 1):
            file.write('\n             ')
            valindex = 0
